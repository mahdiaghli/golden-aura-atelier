...
function ProductCard({ product }: { product: (typeof products)[number] }) {
  ...
  return <Link to="/shop/$id" params={{ id: product.id }} className="group block"><div className="relative overflow-hidden bg-secondary"><img src={product.image} alt={product.name} className="aspect-[4/5] w-full object-cover transition duration-700 group-hover:scale-105" />{product.bestseller && <span className="absolute left-3 top-3 bg-gold px-2 py-1 text-[9px] font-bold uppercase tracking-wider">Best seller</span>}</div><div className="pt-4"><h3 className="font-serif text-xl group-hover:text-gold">{product.name}</h3><p className="mt-1 text-[10px] uppercase tracking-widest text-onyx/50">{product.karat} · {product.weight}g</p><p className="mt-2 text-sm font-medium">{formatToman(total)}</p></div></Link>;
}

function Showcase({ eyebrow, title, items }: { eyebrow: string; title: string; items: typeof products }) {
  return <section className="max-w-7xl mx-auto px-6 py-16 md:py-24"><div className="mb-10 flex items-end justify-between gap-6"><div><p className="text-[11px] uppercase tracking-[.32em] text-gold">{eyebrow}</p><h2 className="mt-3 font-serif text-4xl md:text-5xl">{title}</h2></div><Link to="/shop" className="shrink-0 border-b border-gold pb-1 text-[10px] font-bold uppercase tracking-widest text-gold">Shop all</Link></div><div className="grid grid-cols-2 gap-x-5 gap-y-10 md:grid-cols-4 md:gap-x-7">{items.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>;
}
...
function Home() {
  ...
  return <Shell>
    ...
    <Showcase eyebrow="Just arrived" title="Newest pieces" items={newest} />
    ...
    <Showcase eyebrow="Curated by Aurum" title="Recommended for you" items={recommended} />
    ...
    <Showcase eyebrow="Style it together" title="Complete the look" items={completeTheLook} />
