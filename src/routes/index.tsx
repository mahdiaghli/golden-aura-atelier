  ...
  function ProductCard({ product }: { product: (typeof products)[number] }) {
    ...
    return (
      <Link to="/shop/$id" params={{ id: product.id }} className="group block">
        ...
        <div className="pt-4">
          <h3 className="font-serif text-xl group-hover:text-gold">{localizedProduct.name}</h3>
    ...
    return (
      <Shell>
        <section className="relative min-h-[660px] overflow-hidden">
          <img src="/products/featured-1.jpg" alt={t("home.heroImageAlt")} className="absolute inset-0 h-full w-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-parchment via-parchment/80 to-parchment/10" />
          <div className="relative mx-auto flex min-h-[660px] max-w-7xl items-center px-6">
            <div className="max-w-2xl">
              ...
              <h1 className="mt-5 font-serif text-5xl leading-[1.03] md:text-7xl">{t("home.heroTitle")}</h1>
              <p className="mt-7 max-w-lg text-base leading-relaxed text-onyx/70">{t("home.heroSubtitle")}</p>
              <div className="mt-9 flex flex-wrap gap-4">
                <Link to="/shop" className="bg-onyx px-7 py-4 text-[10px] font-bold uppercase tracking-widest text-parchment hover:bg-gold hover:text-onyx">
                  {t("home.heroCtaShop")}
                </Link>
                <Link to="/signup" className="border border-onyx/30 px-7 py-4 text-[10px] font-bold uppercase tracking-widest hover:border-gold hover:text-gold">
        ...
        <section className="bg-secondary py-16 md:py-24">
          <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[1fr_1.1fr]">
            <div>
              ...
              <h2 className="mt-4 font-serif text-4xl md:text-5xl">{t("home.whyTitle")}</h2>
